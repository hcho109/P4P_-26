...
startButton.addEventListener("click", function(){
    if(!isSpectrogramRunning){
        ...
        time_samples = 800; // X resolution
        n_vertices = (frequency_samples+1) * (time_samples+1);
        xsegments = time_samples;
        ysegments = frequency_samples;
...
function init() {
    ...
    let container = document.getElementById( 'canvas_spectrogram' );
...
if (isSpectrogramRunning) {
    // Update your spectrogram and render the scene here
...
function update_geometry() {
    ...
    // Shift the heights array to the right (left-to-right motion)
    // Shift existing values to the right
    for (let i = n_vertices - 1; i >= frequency_samples + 1; i--) {
        heights[i] = heights[i - frequency_samples - 1];
    }

    // Add new values at the beginning of the array
    for (let i = 0; i < frequency_samples + 1; i++) {
        heights[i] = DATA[i];
    }
